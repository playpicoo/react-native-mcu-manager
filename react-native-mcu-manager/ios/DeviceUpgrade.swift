...
class DeviceUpgrade {
    ...
    init(id: String, bleId: String, fileURI: String, options: UpdateOptions,
    ...
    func startUpgrade(_ promise: Promise) {
        ...
        do {
            ...
            let config = FirmwareUpgradeConfiguration(
                ...
                pipelineDepth: self.options.windowUploadCapacity ?? 1,
                byteAlignment: self.getByteAlignment(),
    ...
    private func getByteAlignment() -> ImageUploadAlignment {
        ...
        guard let jsAlignment = JSMemoryAlignment(rawValue: self.options.memoryAlignment!) else {
            return .disabled
        }
